...
const OnlineStoreProductCard = ({ productData: product }: Props) => {
  ...
  return product ? (
    <Link to={AppUrls.shopOnlineProductDetail(generateNameId({ name: product.name, id: product._id }))}>
      <div className='relative overflow-hidden rounded-lg bg-white p-2 transition-transform duration-500 hover:translate-y-[-0.1rem] hover:shadow-around'>
        ...
        <div className='w-fll group relative overflow-hidden rounded-lg pt-[100%]'>
          ...
          <img
            src={product.image}
            alt=''
            className={classNames('absolute left-0 top-0 z-[1] h-full w-full rounded-lg bg-gray-50 object-cover', {
              'hidden ': imgLoading,
              'block border': !imgLoading
            })}
          ...
          {/* {product?.otherImages?.length && (
            <img
              ...
              alt='other-img'
              className={classNames(
                'absolute left-0 top-0 h-full w-full rounded-lg bg-gray-50 object-cover transition-transform duration-300 group-hover:z-[2] group-hover:scale-110',
                {
                  'hidden ': imgLoading,
                  'block border': !imgLoading
                }
        ...
        <div className='mt-2 overflow-hidden'>
          ...
          <div className='mt-0 flex items-center'>
            <div className='max-w-[90%] truncate text-gray-600'>
              <span className='mr-1 text-sm font-medium text-primary'>$ {product.price}</span>
...
export default OnlineStoreProductCard
