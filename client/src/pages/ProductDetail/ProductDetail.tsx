...
import { SampleProduct, SampleProductDetail } from 'src/types/product.type'
import Slider from 'react-slick'
import BreadCrumbs from 'src/components/BreadCrumbs/BreadCrumbs'
import { AnimatePresence, motion } from 'framer-motion'
import { fadeAnimation, slideAnimation } from 'src/config/motion'
import { useQuery } from '@tanstack/react-query'
import { generateNameId, getIdFromNameId } from 'src/utils/utils'
...
const ProductDetail = (props: Props) => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => productApi.getSampleProductWithId(getIdFromNameId(productId as string)),
    staleTime: 2 * (60 * 1000)
  })
  const product: SampleProductDetail = data?.data?.data
  ...
  return (
    <AnimatePresence>
      {isLoading ? (
        ...
        <motion.section
          {...fadeAnimation}
          className='container grid grid-cols-6 gap-6 overflow-scroll py-4 md:py-6 xl:gap-7'
        >
          ...
          {product && (
            <>
              ...
              <motion.div
                ...
                className='col-span-6 flex flex-col justify-center gap-6 lg:col-span-3'
              >
                <div className=''>
                  <span className='block text-base text-gray-400'>{product?.type.name}</span>
                  <h2 className='mt-2 text-3xl font-semibold text-gray-900'>{product?.name}</h2>
                </div>
                <h3 className='text-2xl text-primary'>$ {product.price}+</h3>
                <ul className='list-disc pl-6 text-base text-gray-900'>
                  {product.details.map((info: string) => (
                    <li key={info}>{info}</li>
                  ))}
                </ul>
